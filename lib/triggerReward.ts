export async function triggerDexReward(
	...
	try {
		...
		if (res.ok) {
			console.log(
				`✅ [Arbitrage Inc] Punti accreditati per l'azione: ${actionType}`,
			);
		} else {
			console.warn(
				`⚠️ [Arbitrage Inc] Server ha rifiutato i punti per ${actionType}`,
			);
		}
	} catch (error) {
		console.error(
			`❌ [Arbitrage Inc] Errore di rete nel sistema reward`,
			error,
		);
	}
}
