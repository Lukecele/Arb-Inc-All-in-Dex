...
test.describe("Limit Orders", () => {
	test("Limit order creation page loads with form", async ({ page }) => {
		...
		await expect(
			page.locator('button:has-text("Preview Order")'),
		).toBeVisible();
	});

	test("Limit order listing page loads with connect wallet", async ({
		page,
	}) => {
	...
	test("Order list shows filter buttons", async ({ page }) => {
		...
		await expect(page.locator('button:has-text("Active")')).toHaveCSS(
			"background-color",
			/rgb/,
		);
	});
