...
test.describe("Arbitrage Inception Website", () => {
	...
	test("Pool selector shows all pools including V3", async ({ page }) => {
		...
		const pcsV2Option = page
			.locator('optgroup[label="PancakeSwap V2"] option')
			.first();
		await expect(pcsV2Option).toBeVisible();

		const pcsV3Option = page
			.locator('optgroup[label="PancakeSwap V3"] option')
			.first();
	...
	test("Zap Out shows correct link format", async ({ page }) => {
		...
		expect(href).toMatch(
			/pancakeswap\.finance\/remove\/.+\/.+|pancakeswap\.finance\/liquidity\/.+/,
		);
	});
});
