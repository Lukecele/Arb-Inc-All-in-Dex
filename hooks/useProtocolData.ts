...
export const useProtocolData = (CONTRACT_ADDRESS: string, TREASURY_WALLET: string, ACCUMULATOR_WALLET: string) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let vol = 0, liq = 0;
        if (dataDex.pairs) {
          vol = dataDex.pairs.reduce((acc: number, pair: any) => acc + (pair.volume?.h24 || 0), 0);
          liq = dataDex.pairs.reduce((acc: number, pair: any) => acc + (pair.liquidity?.usd || 0), 0);
        }

        const rpcUrl = 'https://bsc-dataseed.binance.org/';
        const rpcBody = (method: string, params: any[]) => JSON.stringify({ jsonrpc: '2.0', method, params, id: 1 });
        ...
        const [statsRes, aprRes] = await Promise.all([fetch('/api/stats'), fetch('/api/apr')]);
        ...
        setData(prev => ({
          ...
          tokenVolume24h: vol,
          tokenLiquidity: liq,
          treasuryBnb: dT.result ? (Number(BigInt(dT.result)) / 1e18).toFixed(4) : '...',
          accBnb: dAB.result ? (Number(BigInt(dAB.result)) / 1e18).toFixed(4) : '...',
          accTokens: dAT.result ? (Number(BigInt(dAT.result)) / 1e9).toLocaleString(undefined, {maximumFractionDigits: 0}) : '...',
    ...
    const tInterval = setInterval(() => {
      ...
      if (timeSinceLast >= 0 && timeSinceLast <= 120000) {
        setData(prev => ({ ...prev, isProcessing: true, timerDisplay: 'Processing...' }));
      } else {
        const diff = nextPayout - now;
        const h = Math.floor(diff / 3600000);
        const m = Math.floor((diff % 3600000) / 60000);
        const s = Math.floor((diff % 60000) / 1000);
        setData(prev => ({ ...prev, isProcessing: false, timerDisplay: `${h}h ${m}m ${s}s` }));
      }
    }, 1000);
