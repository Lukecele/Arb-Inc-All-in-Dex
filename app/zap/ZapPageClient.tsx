...
const GlobalStyle = createGlobalStyle` * { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Inter', sans-serif; background: #030014; color: #FFFFFF; } `;
const Container = styled.div` min-height: 100vh; display: flex; flex-direction: column; align-items: center; padding: 40px 20px; background: #030014; @media (max-width: 1024px) { padding: 80px 15px 20px; } `;
const ConnectButton = styled.button` padding: 10px 20px; font-size: 14px; cursor: pointer; background: #a855f7; color: #fff; border: none; border-radius: 12px; font-weight: 600; transition: all 0.2s; &:hover { transform: translateY(-2px); } `;
...
export default function ZapPageClient() {
  ...
  const handleSubmitTx = useCallback(async (txData: any) => {
    ...
    const tx = await signer.sendTransaction({ from: txData.from, to: txData.to, value: txData.value, data: txData.data, gasLimit: txData.gasLimit });
    if (address) {
      const referrer = typeof window !== "undefined" ? window.localStorage.getItem("arb_inc_referrer") || "" : "";
      fetch("/api/dex-reward", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userWallet: address, type: "zap", txHash: tx.hash, referrerWallet: referrer }) });
    }
    return tx.hash;
  }, [wallet, address]);
  ...
  return (
    <>
      <GlobalStyle />
      <Header activePage="/zap" walletSection={address ? (<div style={{ display: "flex", alignItems: "center", gap: "10px" }}><span style={{ fontFamily: "monospace", fontSize: "12px" }}>{address.slice(0, 6)}...{address.slice(-4)}</span><DisconnectButton onClick={() => wallet && disconnect(wallet)}>Disconnect</DisconnectButton></div>) : (<ConnectButton onClick={() => connect()}>Connect Wallet</ConnectButton>)} />
      <Container>
        <MainContent>
          <SectionTitle>Liquidity Zap</SectionTitle>
          <PoolSelector selectedPoolId={selectedPool.id} onPoolChange={setSelectedPool} />
          <div style={{ display: "flex", justifyContent: "center", gap: "15px", marginBottom: "25px" }}>
            <TabButton $active={activeTab === "zap-in"} onClick={() => setActiveTab("zap-in")}>Zap In</TabButton>
            <TabButton $active={activeTab === "zap-out"} onClick={() => setActiveTab("zap-out")}>Zap Out</TabButton>
          </div>
          {activeTab === "zap-in" ? (
            <WidgetWrapper>
            ...
            <WidgetWrapper>
              <ZapOutClient poolAddress={selectedPool.address} poolType={selectedPool.poolType} token0Address={selectedPool.token0.address} token0Symbol={selectedPool.token0.symbol} token1Address={selectedPool.token1.address} token1Symbol={selectedPool.token1.symbol} />
            </WidgetWrapper>
          )}
        </MainContent>
        <Footer />
