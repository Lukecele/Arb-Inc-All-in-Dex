...
export default function ZapPageClient() {
  ...
  const handleSubmitTx = useCallback(async (txData: any) => {
    ...
    const tx = await signer.sendTransaction({
      from: txData.from, to: txData.to, value: txData.value, data: txData.data, gasLimit: txData.gasLimit,
    })
    ...
    if (address) {
      ...
      fetch('/api/dex-reward', {
        ...
        body: JSON.stringify({ userWallet: address, type: 'zap', txHash: tx.hash, referrerWallet: referrer })
      }).then(() => { alert("🎉 Liquidity Zap Successful! +150 Points added!"); });
    }
    return tx.hash
  }, [wallet, address])
        ...
        walletSection={
          address ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ fontFamily: 'monospace', fontSize: '12px' }}>{address.slice(0, 6)}...{address.slice(-4)}</span>
              <DisconnectButton onClick={() => wallet && disconnect(wallet)}>Disconnect</DisconnectButton>
            </div>
          ) : (
          ...
          {activeTab === 'zap-in' ? (
            <WidgetWrapper>
              ...
              <WarningBadge>
                ...
                <div>
                  <strong>Tax Token Notice:</strong> When zapping Arbitrage Inception (ARB INC), please set your slippage to <strong>8%</strong> to ensure the transaction processes successfully due to tokenomics.
                </div>
              </WarningBadge>
            ...
            <WidgetWrapper>
               <ZapOutClient 
                 poolAddress={selectedPool.address} poolType={selectedPool.poolType} token0Address={selectedPool.token0.address} token0Symbol={selectedPool.token0.symbol} token1Address={selectedPool.token1.address} token1Symbol={selectedPool.token1.symbol}
               />
            </WidgetWrapper>
          )}
        </MainContent>
