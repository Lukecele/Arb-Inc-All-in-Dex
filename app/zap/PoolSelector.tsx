...
export default function PoolSelector({
	...
	const selectedPool =
		allPools.find((p) => p.id === selectedPoolId) || allPools[0];
		...
		<SelectorContainer>
			...
			<Select value={selectedPoolId} onChange={handleChange}>
				<optgroup label="📈 Arbitrage Inception">
					{allPools
						.filter((p) => p.isArbitrageInception)
						.map((pool) => (
							<option key={pool.id} value={pool.id}>
								[{getPoolTypeLabel(pool.poolType)}] {pool.name} - $
				...
				<optgroup label="🥞 PancakeSwap V2">
					{allPools
						.filter(
							(p) =>
								!p.isArbitrageInception && p.poolType === "DEX_PANCAKESWAPV2",
						)
						.map((pool) => (
							<option key={pool.id} value={pool.id}>
								[{getPoolTypeLabel(pool.poolType)}] {pool.name} - $
				...
				<optgroup label="🥞 PancakeSwap V3">
					{allPools
						.filter((p) => p.poolType === "DEX_PANCAKESWAPV3")
						.map((pool) => (
							<option key={pool.id} value={pool.id}>
								[{getPoolTypeLabel(pool.poolType)}] {pool.name} - $
								{pool.liquidityUSD.toLocaleString()}{" "}
			...
			<PoolDetails>
				...
				<div style={{ fontSize: "12px" }}>
					Pool: {selectedPool.address.slice(0, 6)}...
					{selectedPool.address.slice(-4)}
					<br />
