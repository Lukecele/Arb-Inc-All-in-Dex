...
const HomePageClient = () => {
  ...
  const data = useProtocolData(CONTRACT_ADDRESS, TREASURY_WALLET, ACCUMULATOR_WALLET);
  ...
  if (!isNaN(debtNum) && !isNaN(treasuryNum) && treasuryNum > 0) {
    ...
    if (ratio <= 55) { statusStr = "✅ SECURE"; statusColor = "#22c55e"; }
    else if (ratio <= 90) { statusStr = "⚠️ WARNING"; statusColor = "#facc15"; }
    else { statusStr = "🚨 DANGER"; statusColor = "#ef4444"; }
  }
      ...
      <S.Container>
        <S.Hero>
          ...
          <S.ContractContainer>
            ...
            <S.AuditBadgeLink href={TOKEN_SNIFFER_LINK} target="_blank" rel="noreferrer">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <S.PulseCard $isProcessing={data.isProcessing}>
              ...
              <div style={{ background: 'rgba(168, 85, 247, 0.05)', border: '1px solid rgba(168, 85, 247, 0.2)', borderRadius: '12px', padding: '15px', marginTop: 'auto' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}><span style={{color: '#94a3b8'}}>Pending ARB</span><span style={{ color: 'white' }}>{data.accTokens} ARB</span></div>
              </div>
            ...
            <S.ActionButton href="/rewards" style={{ display: 'block', width: '100%', boxSizing: 'border-box', padding: '14px', borderRadius: '16px', fontSize: '1rem' }}>Go to Rewards</S.ActionButton>
          </div>
          
          <S.PulseCard>
            <div className="header-row"><span className="label">Trading Volume (24h)</span><S.LiveIndicator><div className="dot"></div>Live</S.LiveIndicator></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', margin: '15px 0' }}>
              <div>
                ...
                <span style={{ color: '#3b82f6', fontWeight: 'bold', fontSize: '1.8rem', display: 'block' }}>
                  {data.tokenVolume24h !== null ? `$${data.tokenVolume24h.toLocaleString(undefined, {maximumFractionDigits: 0})}` : <FaSpinner className="fa-spin" size={20} />}
                </span>
              </div>
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
                ...
                <span style={{ color: 'white', fontWeight: 'bold', fontSize: '1.2rem' }}>
                  {data.tokenLiquidity !== null ? `$${data.tokenLiquidity.toLocaleString(undefined, {maximumFractionDigits: 0})}` : <FaSpinner className="fa-spin" size={14} />}
                </span>
              </div>
            </div>
          ...
          <S.PulseCard>
            ...
            <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', padding: '20px', margin: '15px 0' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <span style={{ color: '#facc15', fontWeight: 'bold', fontSize: '1.8rem' }}>{data.treasuryBnb} BNB</span>
              </div>
            </div>
            <div style={{ background: 'rgba(34, 197, 94, 0.05)', border: '1px solid rgba(34, 197, 94, 0.2)', borderRadius: '12px', padding: '15px', marginTop: 'auto' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginBottom: '6px' }}><span style={{color: '#94a3b8'}}>Total User Debt</span><span style={{ color: '#ef4444', fontWeight: 'bold' }}>{data.protocolDebt} BNB</span></div>
            ...
            <div style={{ background: 'rgba(250, 204, 21, 0.15)', border: '1px solid rgba(250, 204, 21, 0.4)', borderRadius: '12px', padding: '15px', marginTop: '10px', textAlign: 'center' }}>
        ...
        <S.AuditSection>
          ...
          <div className="audit-grid">
            <S.AuditCard><FaCheckCircle className="icon" /><h4>KyberSwap Integration</h4><p>We leverage KyberSwap widgets for trading logic, audited by ChainSecurity.</p></S.AuditCard>
            <S.AuditCard><FaLock className="icon" /><h4>Zero-Contract Risk</h4><p>By avoiding custom swap contracts, we eliminate the primary entry point for hacks.</p></S.AuditCard>
            <S.AuditCard><FaCode className="icon" /><h4>Frontend Rewards Logic</h4><p>Rankings are processed by a transparent frontend engine with 9-decimal precision.</p></S.AuditCard>
          </div>
        </S.AuditSection>
      </S.Container>
      <Footer />
