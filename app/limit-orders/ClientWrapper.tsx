...
const getTokenAddressForContract = (addr: string) => addr.toLowerCase() === NATIVE_BNB_ADDRESS.toLowerCase() ? WBNB_ADDRESS : addr;
const getApiTokenAddress = (addr: string) => addr.toLowerCase() === NATIVE_BNB_ADDRESS.toLowerCase() ? WBNB_ADDRESS : addr;

async function fetchTokenPrices(): Promise<Record<string, number>> {
  const prices: Record<string, number> = {};
  const promises = BSC_TOKENS.map(async (token) => {
    if (token.address.toLowerCase() === USDT_ADDRESS.toLowerCase()) { prices[token.address] = 1; return; }
    try {
      const apiToken = getApiTokenAddress(token.address);
      const res = await fetch(`https://aggregator-api.kyberswap.com/bsc/api/v1/routes?tokenIn=${apiToken}&tokenOut=${USDT_ADDRESS}&amountIn=1000000000000000000`, { headers: { 'x-client-id': 'arb-inc' } });
      const data = await res.json();
      if (data.data?.routeSummary?.amountOutUsd) prices[token.address] = parseFloat(data.data.routeSummary.amountOutUsd);
      else if (data.data?.routeSummary?.amountOut) prices[token.address] = parseFloat(data.data.routeSummary.amountOut) / 1e18;
      else prices[token.address] = 1;
    } catch { prices[token.address] = 1; }
  });
...
async function fetchBalance(tokenAddr: string, wallet: string, provider: any): Promise<string> {
  try {
    ...
    if (tokenAddr === '0x0000000000000000000000000000000000000000') return ethers.utils.formatEther(await ethersProvider.getBalance(wallet));
...
const InputRow = styled.div`display: flex; background: #27272a; border: 1px solid #3f3f46; border-radius: 12px; padding: 10px 12px;`;
...
const TokenButton = styled.button`display: flex; align-items: center; gap: 6px; padding: 6px 10px; background: #18181b; border: 1px solid #3f3f46; border-radius: 10px; color: #fff; font-size: 13px; font-weight: 500; cursor: pointer;`;
...
const formatNumber = (num: string | number) => { const n = typeof num === 'string' ? parseFloat(num) : num; if (isNaN(n)) return '0'; if (n >= 1e6) return (n / 1e6).toFixed(2) + 'M'; if (n >= 1e3) return (n / 1e3).toFixed(2) + 'K'; return n.toFixed(4); };
...
export default function ClientWrapper() {
  ...
  useEffect(() => { fetchTokenPrices().then(setTokenPrices).catch(() => {}); }, []);
  useEffect(() => { if (provider && walletAddress) setMaker(createLimitOrderMaker(getDefaultClient())); else setMaker(null); }, [provider, walletAddress]);

  const loadBalances = useCallback(async () => {
    if (!walletAddress || !provider) return;
    const bals: Record<string, string> = {};
    const prov = new ethers.providers.Web3Provider(provider);
    bals['0x0000000000000000000000000000000000000000'] = ethers.utils.formatEther(await prov.getBalance(walletAddress));
    for (const t of BSC_TOKENS) bals[t.address] = await fetchBalance(t.address, walletAddress, provider);
    setBalances(bals);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await tx.wait();
      setApprovalNeeded(false); alert('Approval successful!');
    } catch (e: any) { alert(e.message || 'Approval failed'); }
    setApproving(false);
  };

  const getMarketRate = () => ((tokenPrices[sellToken.address] || 1) / (tokenPrices[buyToken.address] || 1)).toFixed(6);
  const handleRate = (v: string) => { setRate(v); setUseMarketRate(false); if (sellAmount && v) setBuyAmount((parseFloat(sellAmount) * parseFloat(v)).toFixed(6)); };
  ...
  const handleCreate = async () => {
    ...
    await checkApproval();
    if (approvalNeeded) { alert('Please approve the token first'); return; }
    const sellFloat = parseFloat(sellAmount); const rateFloat = parseFloat(rate);
    if (isNaN(sellFloat) || isNaN(rateFloat) || sellFloat <= 0 || rateFloat <= 0) { alert('Invalid amount or rate'); return; }
  ...
  const selectToken = (t: Token) => { if (showTokenModal === 'sell') setSellToken(t); else setBuyToken(t); setShowTokenModal(null); };
      ...
      {showTokenModal && (
        <Modal onClick={()=>setShowTokenModal(null)}>
          <ModalInner onClick={e=>e.stopPropagation()}>
            ...
            {BSC_TOKENS.map(t => (<TokenItem key={t.address} onClick={()=>selectToken(t)}><TokenName>{t.symbol}</TokenName></TokenItem>))}
          </ModalInner>
        </Modal>
      )}
      <Footer />
