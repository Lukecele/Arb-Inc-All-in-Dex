...
async function repair() {
    ...
    const CORRECT_INDEX = (REAL_BNB * 0.80) / REAL_TOTAL_POINTS;

    await redis.set('rewards:global_index', CORRECT_INDEX.toString());
    await redis.set('leaderboard:total_points_sum:global', REAL_TOTAL_POINTS.toString());

    const allWallets = await redis.zrange('leaderboard:points', 0, -1);
    for (const wallet of allWallets) {
        await redis.set(`rewards:user_index:${wallet.toLowerCase()}`, CORRECT_INDEX.toString());
    }
