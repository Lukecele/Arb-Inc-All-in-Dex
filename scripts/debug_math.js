...
async function runDebug() {
	...
	for (const w of allWallets) {
		if (
			w === "0x66bb01f14229e2179bad84d52a69c0e4628de63f" ||
			w === TOKEN_CONTRACT_ADDRESS
		)
			continue;
		try {
			...
			console.log(
				`  - Punti calcolati dal Watcher: +${puntiBonus.toFixed(2)} pt\n`,
			);
		} catch (e) {}
	}
	console.log(
		"🏁 Diagnostica completata. Nessun dato modificato nel database.",
	);
	process.exit(0);
}
runDebug();
