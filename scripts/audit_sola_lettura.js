...
async function run() {
    ...
    try {
        ...
        console.log("----------------------------------------------------------------------");
        console.log("WALLET          |  TOKEN IN WALLET | PUNTI OGNI 15 MIN | STATUS");
        console.log("----------------------------------------------------------------------");
        ...
        for (const w of topWallets) {
            try {
                ...
                const tokensStr = tokens.toLocaleString(undefined, {maximumFractionDigits: 0}).padEnd(15);
                const pointsStr = `+${pointsPerCycle.toFixed(4)}`.padEnd(17);
                
                console.log(`${shortW} | ${tokensStr} | ${pointsStr} | ${status.toUpperCase()}`);
            } catch (err) {
                console.log(`${w.slice(0,6)}... | ❌ Salto: Problema lettura blockchain`);
            }
        }
    } catch (e) {
