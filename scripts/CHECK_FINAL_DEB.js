...
async function runCheck() {
	...
	if (pointsTreasury) {
		console.log(
			`   ❌ ERRORE: Il Treasury è ancora in classifica con ${pointsTreasury} punti!`,
		);
	} else {
	...
	const top3 = await redis.zrange("leaderboard:points", 0, 2, {
		rev: true,
		withScores: true,
	});
	for (let i = 0; i < top3.length; i += 2) {
		...
		const bal =
			(await redis.get(`rewards:pending:${wallet.toLowerCase()}`)) || "0";
		console.log(
			`   ${i / 2 + 1}. ${wallet}: ${parseFloat(score).toFixed(2)} pts -> ${parseFloat(bal).toFixed(6)} BNB`,
		);
	}
