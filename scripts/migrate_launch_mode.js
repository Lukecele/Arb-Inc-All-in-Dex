...
async function migrate() {
	...
	const LAUNCH_DISTRIBUTION = TOTAL_TREASURY * 0.5; // Distribuiamo solo lo 0.0044
	...
	try {
		...
		for (const data of walletData) {
			...
			console.log(
				`✅ ${data.wallet}: ${currentShow.toFixed(6)} BNB (Stato: Blindato)`,
			);
		}
		...
		console.log(
			`🛡️ Riserva per Gas e Crescita Punti: ${(TOTAL_TREASURY - LAUNCH_DISTRIBUTION).toFixed(6)} BNB`,
		);
	} catch (e) {
		console.error(e);
	}
