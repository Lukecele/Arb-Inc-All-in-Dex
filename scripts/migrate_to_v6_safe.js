...
async function migrate() {
	...
	const SAFE_DISTRIBUTION = TOTAL_TREASURY * 0.7; // 0.00616 BNB distribuiti subito
	...
	try {
		...
		for (const data of walletData) {
			...
			await redis.set(
				`rewards:user_index:${data.wallet}`,
				newUserIndex.toString(),
			);

			const currentShow =
				data.points * (parseFloat(globalIndex) - newUserIndex);
			console.log(
				`✅ ${data.wallet}: Mostra ${currentShow.toFixed(6)} BNB (Margine crescita attivo)`,
			);
		}

		console.log(`\n💰 Totale Promesso: ${SAFE_DISTRIBUTION.toFixed(6)} BNB`);
		console.log(
			`⛽ Riserva Gas + Crescita: ${(TOTAL_TREASURY - SAFE_DISTRIBUTION).toFixed(6)} BNB`,
		);
	} catch (e) {
		console.error(e);
	}
