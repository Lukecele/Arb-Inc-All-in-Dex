...
const TREASURY_WALLET = "0x66BB01F14229E2179bAD84D52A69C0e4628dE63f".toLowerCase();
...
async function checkHealth() {
    ...
    try {
        // 1. Cassa Reale
        const treasuryBalWei = await provider.getBalance(ethers.getAddress(TREASURY_WALLET));
        ...
        if (healthPct > 50 && healthPct < 80) status = "⚠️ ATTENZIONE (Erosione gas fees)";
        if (healthPct >= 80) status = "🚨 PERICOLO (Rischio insolvenza)";

        // 4. Stampa Report
        console.log(`🏦 Cassa Reale (Treasury):  ${treasuryBnb.toFixed(6)} BNB`);
        console.log(`💸 Debito Totale Utenti:    ${totalDebt.toFixed(6)} BNB (su ${activeWallets} wallet)`);
        console.log(`🩺 Indice di Rischio:       ${healthPct.toFixed(2)}%`);
        console.log(`🛡️ Stato Protocollo:        ${status}`);
        
        // 5. Check Punti Rapido
        const top1 = await redis.zrange('leaderboard:points', 0, 0, { rev: true, withScores: true });
        if (top1.length === 0 || top1[1] === 0) {
            console.log("---------------------------------------------------------");
            console.log("⏳ PUNTI: Attualmente a zero. Aspetta il prossimo ciclo del Watcher per la nuova classifica corretta.");
        }
