...
async function scan() {
	...
	setInterval(async () => {
		try {
			const currentPoints =
				(await redis.zscore("leaderboard:points", MY_WALLET)) || "0";
			if (currentPoints !== lastPoints) {
				const diff = parseFloat(currentPoints) - parseFloat(lastPoints);
				console.log(
					`🚨 [${new Date().toLocaleTimeString()}] INTRUSIONE RILEVATA! Punti modificati: ${diff > 0 ? "+" : ""}${diff.toFixed(2)}. Totale ora: ${currentPoints}`,
				);
