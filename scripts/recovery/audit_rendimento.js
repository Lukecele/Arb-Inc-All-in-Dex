...
const watcherCode = fs.readFileSync(
	path.resolve(__dirname, "watcher.js"),
	"utf8",
);
const tokenMatch = watcherCode.match(
	/TOKEN_ADDRESS\s*=\s*["'](0x[a-fA-F0-0]+)["']/,
);
...
async function runAudit() {
	if (!TOKEN_ADDRESS)
		return console.log("❌ Non ho trovato TOKEN_ADDRESS in watcher.js");
	...
	for (const w of topWallets) {
		try {
			...
			console.log(
				`${shortW} | ${tokens.toLocaleString().padEnd(15)} | +${pointsPerCycle.toFixed(2).padEnd(10)} | ${status.toUpperCase()}`,
			);
		} catch (e) {
