...
async function restore() {
	...
	const globalIndex = parseFloat(
		(await redis.get("rewards:global_index")) || "0.000000499747",
	);
	...
	if (!exists) {
		...
		console.log(
			`✅ Utente ripristinato con successo: ${USER_POINTS} Punti e ${correctPending.toFixed(6)} BNB.`,
		);
	} else {
		console.log(
			"✅ L'utente è già al sicuro nel database. Nessun danno fatto.",
		);
	}
